...
class ModelBuilder:
    def __init__(self, config_path="config/params.yaml"):
        with open(config_path, "r") as file:
            ...
            if not os.getenv("CI", "").lower() == "true" and not os.getenv(
                "PYTEST_CURRENT_TEST"
            ):
                try:
                    ...
                    logging.warning(
                        f"Failed to set MLflow tracking URI from Azure ML: {e}"
                    )
            else:
    ...
    def transfer_learning(self, model, train_dataset, val_dataset, class_weights):
        try:
            ...
            optimizer = tf.keras.optimizers.Adam(
                learning_rate=self.config["transfer_learning_lr"]
            )
    ...
    def test_model(self, model, test_dataset):
        try:
            ...
            try:
                os.makedirs(
                    os.path.dirname(self.config["keras_model_path"]),
                    exist_ok=True,
                )
