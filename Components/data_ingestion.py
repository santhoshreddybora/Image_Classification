...
class DataIngestion:
    ...
    def download_and_extract_data(
        ...
        try:
            ...
            blob_client = blob_service_client.get_blob_client(
                container=container_name, blob=params["blob_name"]
            )
            ...
            for blob in blob_service_client.get_container_client(
                ...
                if blob.name == params["blob_name"]:
                    with open(
                        os.path.join(params["data_zip_file_path"], blob.name), "wb"
                    ) as download_file:
                        download_file.write(blob_client.download_blob().readall())
                    print(f"Downloaded {blob.name} to {params['data_zip_file_path']}")
            logging.info(
                f"Downloaded {params['blob_name']} to {params['data_zip_file_path']}"
            )
    ...
    def unzip_data(
        ...
        try:
            zip_file_path = os.path.join(
                params["data_zip_file_path"], params["blob_name"]
            )
    ...
    def initiate_data_ingestion(self):
        try:
            ...
            logging.info(
                f"Data ingestion artifact created with path: {dataingestionartifact.Unzipped_data_path}"
            )
