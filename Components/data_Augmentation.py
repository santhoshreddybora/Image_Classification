...
class DataAugmentation:
    def __init__(
        ...
        self.val_aug = A.Compose([A.Resize(224, 224), A.Normalize()])
    ...
    def all_files_and_labels(self, unzipped_data_path):
        ...
        try:
            ...
            logging.info(
                f"Found {len(all_files)} images across {len(classes)} classes."
            )
        ...
        try:
            if ext == ".dcm":
                ...
                if len(image.shape) == 2:
                    ...
                    logging.error(
                        f"Unexpected image shape: {image.shape} for DICOM file {image_path}"
                    )
            else:
                ...
                if image is None:
                    ...
                    logging.error(
                        f"Unexpected image shape: {image.shape} for file {image_path}"
                    )
            return image
        except Exception as e:
    ...
    def preprocess_image(self, path_str, label, augment=True):
        try:
            ...
            return np.zeros((224, 224, 3), dtype=np.float32), np.int64(
                label
            )  # fallback image

    def tf_augment(self, augment=True):
        def wrapper(path, label):
            def _pyfunc_wrapper(p, label):
                decoded_path = p.numpy().decode("utf-8")
                return self.preprocess_image(decoded_path, label, augment)

            img, label = tf.py_function(
                _pyfunc_wrapper, [path, label], [tf.float32, tf.int64]
            )
            img.set_shape([224, 224, 3])
            label.set_shape([])
            logging.info("Processed image and labels")
            return img, label
    ...
    def initiate_data_augmentation(self):
        ...
        try:
            ...
            logging.info(f"Unzipped data path: {unzipped_data_path}")
            X_train, X_test, y_train, y_test = self.all_files_and_labels(
                unzipped_data_path
            )
            ...
            train_dataset = self.create_datasets(
                X_train,
                y_train,
            ...
            val_dataset = self.create_datasets(
                X_test,
                y_test,
                batch_size=self.config["batch_size"],
                augment=False,
            ...
            logging.info(
                f"Train dataset: {train_dataset}, Validation dataset: {val_dataset}"
            )
            ...
            logging.info(
                "Data augmentation artifact created with train and validation datasets."
            )
